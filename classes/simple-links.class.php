...
class simple_links extends mat_post_type_tax{
	...
	function __construct(){
		...
		add_action( 'init', array( $this, 'link_categories' ) );
                add_action( 'init', array( $this, 'link_tags' ) );
	...
	function additional_fields_meta_box_output($post){
		...
		if( is_array( $names ) ){
		   ...
		   if( current_user_can($simple_links_admin_func->cap_for_settings)){
                    ...
                    <a href="edit.php?post_type=simple_link&page=simple-link-settings">Settings</a>
			  														</p>';
	...
	function get_link_categories( $postID, $full_array = false ){
        ...
        function get_link_tags( $postID, $full_array = false ){
		$tags = get_the_terms( $postID, 'simple_link_tag' );
	...
	function link_categories(){
               self::register_taxonomy( 'simple_link_category', 'simple_link',
                        array(
                            'labels' => array(
	...
	function link_tags(){
               self::register_taxonomy( 'simple_link_tag', 'simple_link',
                        array(
                            'labels' => array(
                            'name'             => 'Link Tags',
       ...
       function get_simple_link_posts($cat_slug) {
             ...
             while ( $query->have_posts() ) : $query->the_post();
                    ...
                    $my_id = get_the_ID();
                    $post_item['id'] = $my_id;
                    $post_item['title'] = get_the_title($my_id);
                    $post_item['url'] = get_post_meta($my_id,'web_address', true);
                    $my_posts[] = $post_item; 
             endwhile;
              return $my_posts;
	...
	function post_type(){
		$this->register_post_type( 'simple_link' , 
                                           array(
                                           'post_type' =>'simple_link',
                                           'pluralTitle'=>'Pillar Page Links',    
