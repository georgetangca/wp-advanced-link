...
var SLcatPage = {
   ...
   init: function( good ){
      ...
      $('#tag-slug, label[for="tag-slug"]').remove();
...
var SLsortPage = {
		...
		init : function(){
		    ...
		    $( '.draggable-children' ).sortable( {
                            ...
                            stop: function(){SLsortPage.sort( $(this).attr('id') );}
		    });
		    
		    
		    //the filter by Categories
		    $('#SL-sort-cat').change( function(){
		    	SLsortPage.catFilter( $(this).val() );
		    });
		...
		sort :function( linkID ){
	    	...
	    	$.post(SLajaxURL.sortURL, data, function(respon){
	    	//alert(respon);
	    	});
...
var SLeditPage = {
		...
		validate : function( text ){
			...
			if(RegExp.test(text)){
				...
				$('.simple-links-input[name="web_address"]').css({
				    'background':'white',
				    'color':'black',
				    'font-weight':'normal'
				});
		...
		fImage : function(){
			var pID = $("#post_ID").val();
			var link = $( "#postimagediv a");
			link.livequery( function(){
				//Change the html based on the button
				 if( $(this).html() == 'Remove featured image' ){
					 $(this).html( 'Remove Link\'s Image' );	
					 $(this).addClass('button-secondary');
				 } else{
					 if( $(this).html() == 'Set featured image' ){
						 $(this).html( 'Add an Image' );
						 $(this).addClass('button-secondary');
						 $(this).attr( {href: '/wp-admin/media-upload.php?post_id='+pID+'&type=image&post_type=simple_links&TB_iframe=true'});
				     }
				}
			});
			
			//Change the title
			$('#postimagediv h3.hndle span').html('Link\'s Image');
