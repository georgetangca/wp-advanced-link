...
var SLcatPage = {
   ...
   init: function( good ){
      ...
      jQuery('#tag-slug, label[for="tag-slug"]').remove();
      jQuery('#edittag input[id="slug"], label[for="slug"], #edittag p.description').remove();
  }   
...
var SLsettings = {
		init: function(){
			...
			jQuery('#simple-link-additional').click( function(){
				jQuery('#link-additional-placeholder').after( $('#link-extra-field').html() );
				return false;
			});
		}
}
...
var SLsortPage = {
		...
		init : function(){
		    ...
		    jQuery( '.draggable-children' ).sortable( {
                            placeholder: 'sortable-placeholder menu-item-depth-1',
                            stop: function(){SLsortPage.sort( jQuery(this).attr('id') );}
		    });
		    
		    
		    //the filter by Categories
		    jQuery('#SL-sort-cat').change( function(){
		    	SLsortPage.catFilter( jQuery(this).val() );
		    });
		...
		sort :function( linkID ){
	    	...
	    	var data = jQuery( 'ul#'+linkID ).sortable("serialize");
	    	
	    	//alert( data );
	    	
	    	jQuery.post(SLajaxURL.sortURL, data, function(respon){
	    	//alert(respon);
	    	});
	    ...
	    catFilter : function(slug){
	    	...
	    	if( slug == 'Xall-catsX' ){
	    		jQuery('#SL-drag-ordering li').show('slow');
	    		jQuery('#SL-drag-ordering li').each( function(){
		    		cleanID = jQuery(this).attr('id').replace(/x/g,'');
		    		jQuery(this).attr({'id': cleanID});
		    	});
	    	...
	    	jQuery('#SL-drag-ordering li.'+slug).show('slow');
	    	jQuery('#SL-drag-ordering li.'+slug).each( function(){
	    		cleanID = jQuery(this).attr('id').replace(/x/g,'');
	    		jQuery(this).attr({'id': cleanID});
	    	});
	    	
	    	//Hide and break the id of all the list items not in this category
	    	jQuery('#SL-drag-ordering li').not('.'+slug).hide('slow');
	    	jQuery('#SL-drag-ordering li').not('.'+slug).each( function(){
	    		jQuery(this).attr({'id': 'x'+jQuery(this).attr('id')});
	    	});
...
var SLeditPage = {
		...
		init : function( isSimpleLinks ){
			if( isSimpleLinks ){
				...
				jQuery('#preview-action, #save-action').remove();
				//remove the permalink
				jQuery('#titlediv .inside').remove();
				
				//To Validate the Web Address Text Field
				jQuery('.simple-links-input[name="web_address"]').change( function(){
					SLeditPage.validate( jQuery(this).val() );
				});
		...
		validate : function( text ){
			...
			if(RegExp.test(text)){
				...
				jQuery('.simple-links-input[name="web_address"]').css({
				    'background':'white',
				    'color':'black',
				    'font-weight':'normal'
				});
				...
				jQuery('#publishing-action').off('click');
				...
				//Disable the submit button and add the message
				jQuery('#publish').attr({'disabled':'disabled'});
				jQuery('#publishing-action').click( function(){
					alert( 'You must fix the bright yellow line(s) before you can update this link' );
				});
			}
		...
		fImage : function(){
			...
			var link = jQuery( "#postimagediv a");
			link.livequery( function(){
				//Change the html based on the button
				 if( jQuery(this).html() == 'Remove featured image' ){
					 jQuery(this).html( 'Remove Link\'s Image' );	
					 jQuery(this).addClass('button-secondary');
				 } else{
					 if( jQuery(this).html() == 'Set featured image' ){
						 jQuery(this).html( 'Add an Image' );
						 jQuery(this).addClass('button-secondary');
						 jQuery(this).attr( {href: '/wp-admin/media-upload.php?post_id='+pID+'&type=image&post_type=simple_links&TB_iframe=true'});
				     }
				}
			});
			
			//Change the title
			jQuery('#postimagediv h3.hndle span').html('Link\'s Image');
